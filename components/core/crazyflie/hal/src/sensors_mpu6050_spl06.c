 ...
 *
 * Sensor HAL for pyDrone (01Studio): MPU6050 (IMU) + SPL06-001 (barometer).
 *
 * Adapted from sensors_mpu6050_hm5883L_ms5611.c. The SPL06 sits on the
 * MPU6050 auxiliary I2C bus, so it is read through the MPU6050 in I2C
 * master mode (same scheme the original used for MS5611). The barometer
 * read path is ported from the pyDrone MicroPython port.
 *
 * Compass (QMC5883L) and the VL53L1X/PMW3901 deck are NOT enabled here:
 * pyDrone has none of those, it uses the SPL06 barometer for altitude.
 */
#include <math.h>
 ...
 * multiple slave reading through MPU6050 (MAG and BARO), only single for some reason.
 */
 ...
 * pyDrone: only the SPL06 barometer. No magnetometer, no VL53/PMW3901 deck.
 */
...
// SPL06 is read through the MPU6050 master: 1 status byte + 6 data bytes
...
bool sensorsMpu6050Spl06AreCalibrated()
{
...
/* SPL06 read through MPU6050 master mode.
 * buffer[0]       = SPL06 mode/status register (slave 2)
...
#ifdef SENSORS_ENABLE_PRESSURE_SPL06
...
#ifdef SENSORS_ENABLE_PRESSURE_SPL06
    if (isBarometerPresent) {
        // SPL06 read through MPU6050 master: status (1 byte) + pressure/temp (6 bytes).
        // SPL06 auto-increments from SPL06_PRESSURE_MSB_REG through the temperature regs.
